...
void Basis::sync(const MPI_Comm &comm)
{
    ...
    if (num_proc > 1)
    {
        MPI_Bcast(&num_centers, 1, MPI_INT, 0, comm);
        MPI_Bcast(&num_shells,  1, MPI_INT, 0, comm);
        ...
        MPI_Bcast(l_quantum_num, num_shells, MPI_INT, 0, comm);
        ...
        MPI_Bcast(center_xyz, 3*num_centers, MPI_DOUBLE, 0, comm);
        ...
        MPI_Bcast(center_element, num_centers, MPI_INT, 0, comm);
        ...
        MPI_Bcast(shell_center, num_shells, MPI_INT, 0, comm);
        ...
        MPI_Bcast(shell_center_xyz, 3*num_shells, MPI_DOUBLE, 0, comm);
        ...
        MPI_Bcast(shell_extent_squared, num_shells, MPI_DOUBLE, 0, comm);
        ...
        MPI_Bcast(cartesian_deg, num_shells, MPI_INT, 0, comm);
        ...
        MPI_Bcast(shell_off, num_shells, MPI_INT, 0, comm);
        ...
        MPI_Bcast(spherical_deg, num_shells, MPI_INT, 0, comm);

        MPI_Bcast(&is_spherical, 1, MPI_INT, 0, comm);

        MPI_Bcast(&num_ao,           1, MPI_INT, 0, comm);
        MPI_Bcast(&num_ao_cartesian, 1, MPI_INT, 0, comm);
        MPI_Bcast(&num_ao_spherical, 1, MPI_INT, 0, comm);
        MPI_Bcast(&num_ao_slices,    1, MPI_INT, 0, comm);
        ...
        MPI_Bcast(ao_center, num_ao, MPI_INT, 0, comm);
        ...
        MPI_Bcast(shell_num_primitives, num_shells, MPI_INT, 0, comm);

        MPI_Bcast(&geo_diff_order, 1, MPI_INT, 0, comm);

        int array_length = pow(geo_diff_order + 1, 3);

        if (rank != 0)
        {
            block_size = array_length*sizeof(int);
            geo_off = (int*) MemAllocator::allocate(block_size);
        }
        MPI_Bcast(geo_off, array_length, MPI_INT, 0, comm);
        ...
        MPI_Bcast(primitive_exp, n, MPI_DOUBLE, 0, comm);
        ...
        MPI_Bcast(contraction_coef, n, MPI_DOUBLE, 0, comm);
